...
import jp.co.soramitsu.substrate_sdk.exceptions.Bip39Exception
...
actual class MnemonicCreator {

    actual fun randomMnemonic(length: Mnemonic.Length): Mnemonic = SecureCharBuffer().use { secure ->
    ...
    actual fun fromWords(words: String): Mnemonic {
    ...
    actual fun fromEntropy(entropy: ByteArray): Mnemonic {
    ...
    private fun generateEntropy(mnemonicWords: String): ByteArray {
        ...
        val bits = words.joinToString(separator = "") {
            val index = EnglishWordList.INSTANCE.indexOf(it)
